# Multi-Digital Twin (MDT) Wind Power Forecasting — Complete Project Report

> **Project Type:** B.Tech Final Year Project (BTP)
> **Location:** Sabarmati Riverfront, Ahmedabad, India (SiteID: 36565)
> **Date:** May 2026
...
flowchart TD
    ...
    C -->|train.py| D1["🧠 LSTM\n3 layers, 128 hidden"]
    C -->|train.py| D2["🧠 GRU\n3 layers, 128 hidden"]
    C -->|train.py| D3["🧠 LSTM-CNN\n2 LSTM + 2 Conv1D + Attention"]
    C -->|train.py| D4["🧠 GRU-CNN\n2 GRU + 2 Conv1D + Attention"]
...
Charts convert normalized [0,1] values to real kW in JavaScript:

```javascript
const WP_MIN = -0.6, WP_RANGE = 82.238;
const toKW = v => v * WP_RANGE + WP_MIN;
// Y-axis now shows 0–80 kW instead of 0–1
```
...
|---|---|---|---|---|
| **GRU** | **3.2349** | **4.7339** | 9.43 | **0.9340** |
| LSTM | 3.2955 | 4.7527 | 9.89 | 0.9334 |
| LSTMCNN | 3.7340 | 5.3728 | 11.19 | 0.9149 |
| GRUCNN | 3.9065 | 5.3909 | **7.50** | 0.9144 |

### Table 5 — Method 1: Single-Index Dynamic Optimization

| Group | Combination | MAE (kW) | RMSE (kW) | MAPE (%) | R² |
|---|---|---|---|---|---|
| 2-DT | **GRU&GRUCNN** | **3.2441** | 4.8296 | **8.12** | 0.9315 |
| 2-DT | LSTM&GRU | 3.2721 | **4.7604** | 9.56 | **0.9334** |
| 3-DT | GRU&LSTMCNN&GRUCNN | 3.2954 | 4.9133 | 8.48 | 0.9291 |
| 3-DT | LSTM&GRU&GRUCNN | 3.2931 | 4.8804 | 8.54 | 0.9300 |
| 4-DT | **All Four** | 3.3193 | 4.9339 | 8.72 | 0.9285 |
...
| 2-DT | **GRU&GRUCNN** | **3.2441** | 4.8296 | **8.12** | 0.9315 |
| 2-DT | LSTM&GRU | 3.2721 | **4.7604** | 9.56 | **0.9334** |
| 3-DT | LSTM&GRU&GRUCNN | 3.2706 | 4.8562 | 8.45 | 0.9307 |
| 3-DT | GRU&LSTMCNN&GRUCNN | 3.3084 | 4.9360 | 8.45 | 0.9284 |
| 4-DT | **All Four** | 3.3193 | 4.9143 | 8.71 | 0.9290 |

### Comparison with Reference Paper

| Metric | Our Results | Reference Paper |
|---|---|---|
| Best Single DT MAE | 3.23 kW | 2.54 kW |
| Best Single DT RMSE | 4.73 kW | 4.51 kW |
| Best Single DT R² | **0.9340** | 0.8895 |
| Best MDT MAE | 3.24 kW | 2.39 kW |
| Best MDT MAPE | **8.12%** | — |
| Best MDT R² | **0.9334** | 0.8990 |
